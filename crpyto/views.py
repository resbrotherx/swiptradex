...
def deposit(request):
	if request.method == 'POST':
		...
		html_template = 'crypto/deposit_massage.html'
		html_message = render_to_string(html_template, context=mydict)
		subject = 'Deposit From' + '' + str(pode.user)
		email_from = settings.EMAIL_HOST_USER
		recipient_list = ['payment@lifemapcapital.com', email]
	...
	for postt in user_postt:
		pos = postt.status
	waladd = wallet_address.objects.all()
	context = {
	'post':post,
	'waladd':waladd
	}
...
def parkages(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			settings.EMAIL_HOST_USER,
			# [settings.EMAIL_HOST_USER],
			['admin@lifemapcapital.com','support@lifemapcapital.com']
			)
			messages.success(request, 'Your request has been add')
	...
	for postt in user_postt:
		pos = postt.status
	return render(request, 'crypto/parkages.html',{'pk':user_packages.filter(appruved=True),'pks':user_package.filter(appruved=False),"count":user_packages_count,'form':form,'pack':pack})

def speacial_parkages(request):
	if request.method == 'POST':
		#print('Printing POST:', request.POST)
		form = MembershipForm(request.POST)
		if form.is_valid():
	...
	for postt in user_postt:
		pos = postt.status
	return render(request, 'crypto/speacial_package.html',{'pk':user_packages.filter(appruved=True),'pks':user_package.filter(appruved=False),"count":user_packages_count,'form':form})
...
def approvalmail(request, id):  
	...
	if request.method == 'POST':
		...
		html_template = 'crypto/trans_massage.html'
		html_message = render_to_string(html_template, context=mydict)
		subject = 'Lifemapcapital'
		email_from = settings.EMAIL_HOST_USER
		recipient_list = [email, 'admin@lifemapcapital.com']
		message = EmailMessage(subject, html_message,
								   email_from, recipient_list)
		message.content_subtype = 'html'
		message.send()
		...
		# 	"amount":amount
		# })
		# send_mail('From Lifemapcapital',
		# template,
		# settings.EMAIL_HOST_USER,
		# [email,'admin@lifemapcapital.com'],
		# )
...
def userall(request, pk):
	order = UserWallet.objects.get(id=pk)
	return render(request, 'crypto/all_user_detail_page.html', {'employee': order})
...
def auto(request):
	user_packages = UserMembership.objects.filter(appruved=True).order_by('-item_created_date')
	for membership in user_packages:
		pakage = membership.membership
		price = float(membership.amount)
		uzer = membership.user
		roi = membership.roi
		# return HttpResponse(pakage)
		try:
			discout = (float(roi) / 100) * price
			user_wall = UserWallet.objects.filter(user=uzer)
			for bal in user_wall:
				ballan = bal.profit
			balance_amount = float(ballan)
			amountes = float(discout)
				# return HttpResponse(str(amountes) + ' ' + 'wow ' + str(pakage))
			deposited = balance_amount + amountes
			forms = UserWallet.objects.filter(user=uzer).update(profit=deposited)
			...
			# 		# return HttpResponse(str(amountes) + ' ' + 'wow ' + str(pakage))
			# 	deposited = balance_amount + amountes
			# 	UserWallet.objects.filter(user=uzer).update(profit=deposited)
			# 	event = form.save(commit=False)
			# 	event.user = uzer
			# 	event.save()
			# 	return HttpResponse("Successfully")
			# elif pakage == "Executive":
			# 	profitc = float('2.9')
			# 	discout = (profitc / 100) * price
			# 	user_wall = UserWallet.objects.filter(user=uzer)
			# 	for bal in user_wall:
			# 		ballan = bal.profit
			# 	balance_amount = float(ballan)
			...
			# 	event.save()
			# 	return HttpResponse("Successfully")
			# elif pakage == "Ultimate":
			# 	profitd = float(5.0)
			# 	discout = (profitd / 100) * price
			# 	user_wall = UserWallet.objects.filter(user=uzer)
			# 	for bal in user_wall:
			# 		ballan = bal.profit
			# 	balance_amount = float(ballan)
			...
			# 	event.save()
			# 	return HttpResponse("Successfully")
			# elif pakage == "Platinum":
			# 	profite = float(7.1)
			# 	discout = (profite / 100) * price
			# 	user_wall = UserWallet.objects.filter(user=uzer)
			# 	for bal in user_wall:
			# 		ballan = bal.profit
			# 	balance_amount = float(ballan)
...
def clientip(request):
	ip = User_ip.objects.all()
	context = {
		"ip":ip
	}
	return render(request,'crypto/allclientip.html',context)
...
def about(request):
    return render(request,'crypto/about-page.html')
...
def terms(request):
	return render(request,'crypto/tearmsandcondition.html')

def privacy(request):
	return render(request,'crypto/privacy.html')
...
def Chat(request):
	return render(request,'crypto/chat.html')